...
func symlinkChtimes(linkPath string, btime, atime, mtime time.Time) error {
	return setFileTimes(linkPath, btime, atime, mtime, true, true)
}
...
func setFileTimes(path string, btime, atime, mtime time.Time, isSymlink bool, fallbackToMtime bool) error {
	// Convert times to Windows FILETIME format
	ftc := windows.NsecToFiletime(btime.UnixNano())
	if btime.IsZero() && fallbackToMtime {
